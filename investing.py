...
def args():
    ...
    if len(sys.argv) == 2 and sys.argv[1] in ['-h', '--help']:exit()
    ...
    parser.add_argument('-f', '--fund', default=1000, type=float)
    parser.add_argument('-e', '--end', default='2026-02-25 13:05:03')
    parser.add_argument('-c', '--clip', default=2, type=int)
    parser.add_argument('-w', '--win', default=30, type=int)
...
def datetime_to_block(ts_str: str) -> int:
    if not ts_str:
        return BASE_BLOCK
    base_dt = datetime.strptime(BASE_TIME_STR, '%Y-%m-%d %H:%M:%S')
    dt = datetime.strptime(ts_str, '%Y-%m-%d %H:%M:%S')
    delta_sec = (dt - base_dt).total_seconds()
    return BASE_BLOCK + int(delta_sec / BLOCK_SECONDS)
...
def initfund(self):
    ...
    if {'date', 'time'}.issubset(st.columns):
        dt_series = self.st['date'].astype(str) + ' ' + self.st['time'].astype(str)
        start_ts = dt_series.min()
        start_block = datetime_to_block(start_ts)
    else:
        start_block = None
    ...
    for _, di in st.sort_values(['date', 'block']).iterrows():
        ...
        block = bn[bn['block'] >= start_block]['block'].iat[0] if notin else di['block']
...
def pldaily(self, date, a=0):
    ...
    if {'date', 'time'}.issubset(self.st.columns):
        dt_series = self.st['date'].astype(str) + ' ' + self.st['time'].astype(str)
        start_ts = dt_series.min()
        start_block = datetime_to_block(start_ts)
    ...
    end_block = datetime_to_blocks(end_date)
    ...
    elif date_format.date() == end_date.date():
...
def plfinal(self):
    ...
    self.hl = hl = self._hl
    self.ba = hl[(hl['alpha_close'].abs() > 1e-6) | (hl.loc[:,'block_open':].abs().sum(axis=1) == 0)][self.ba.columns]
...
def main():
    close_time = datetime(2026, 2, 24, 13, 5, 3)
    strat = calculate_division(close_time)
